  ...
  function(details) {
    ...
    if (details.url.includes('api.claude.ai/api/organizations/') && 
      ...
      .then(data => {
        ...
        console.log('Conversation data fetched successfully:', JSON.stringify(data).substring(0, 100) + '...');
      })
      .catch(error => {
...
chrome.runtime.onMessage.addListener((request, sender, sendResponse) => {
  ...
  if (request.action === "getConversationData") {
    console.log('Sending conversation data:', conversationData ? 'Data available' : 'No data');
